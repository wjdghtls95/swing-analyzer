...
def iter_from_manifest(manifest_path: Path):
    ...
    for item in data:
        yield (
            item["path"],
            item.get("pro") or item.get("athlete", "unknown"),
            item.get("club", "unknown"),
        )
...
def analyze_one(
    video_path: str, club: str, side: str, min_vis: float, norm_mode: str, timeout: int
):
...
def main():
    ...
    ap.add_argument(
        "--root",
        type=str,
        help="영상이 들어있는 루트 디렉토리 (manifest 없을 때 폴더 스캔)",
    )
    ...
    for video_path, athlete, club in tqdm(src_iter, desc="Analyzing"):
        try:
            res = analyze_one(
                video_path, club, args.side, args.min_vis, args.norm_mode, args.timeout
            )
