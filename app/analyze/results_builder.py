...
def build_result(
    *,
    ...
    _maybe_put(result, "pose", {
        "frameStep": 3,
        "minVisibility": min_vis,
    })
    _maybe_put(result, "phase", {
        "method": phase_method,
    })
    ...
    # 결과 본문 (값이 있을 때만)
    if metrics:          _maybe_put(result, "metrics", metrics)
    if phases:           _maybe_put(result, "phases", phases)
    if phase_metrics:    _maybe_put(result, "phase_metrics", phase_metrics)
