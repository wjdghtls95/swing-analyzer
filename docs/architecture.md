# 🏗️ Architecture

## 전체 시스템 구조

```
┌─────────────────────────────────────────────────────────────┐
│                    Golf Platform Ecosystem                   │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  [Client Mobile App]                                        │
│           ↓                                                 │
│  ┌─────────────────────────────────────┐                   │
│  │   NestJS Backend API                │                   │
│  │   (platform repo)                   │                   │
│  │   - 사용자 인증 (Google OAuth)        │                   │
│  │   - 골프장 검색 (Redis GEO) etc..     │                   │
│  │   - 스윙 분석 요청 관리                 │                   │
│  │   - 결과 DB 저장                      │                   │
│  └─────────────────────────────────────┘                   │
│           ↓ HTTP POST /analyze                             │
│  ┌─────────────────────────────────────┐                   │
│  │   FastAPI AI Service (이 레포)       │                   │
│  │   - 영상 전처리 (FFmpeg)             │                   │
│  │   - MediaPipe 골격 추출              │                   │
│  │   - LSTM Phase 감지                  │                   │
│  │   - 각도 계산 & 진단                  │                   │
│  └─────────────────────────────────────┘                   │
│           ↓ JSON Response                                  │
│  ┌─────────────────────────────────────┐                   │
│  │   MySQL (RDS)                       │                   │
│  │   - 사용자 데이터                     │                   │
│  │   - 분석 결과 저장                    │                   │
│  └─────────────────────────────────────┘                   │
│                                                             │
│  ┌─────────────────────────────────────┐                   │
│  │   Redis (ElastiCache)               │                   │
│  │   - 골프장 위치 데이터 (GEO)          │                   │
│  │   - 세션 캐시                         │                   │
│  └─────────────────────────────────────┘                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
         ▲
         │ Terraform 관리
         │
   ┌─────────────────┐
   │ platform-infra  │
   │   (IaC 코드)    │
   └─────────────────┘
```

---

## 이 서비스의 역할

### ✅ 담당하는 것
- 영상 전처리 (FFmpeg 표준화)
- 골격 추출 (MediaPipe Pose)
- Phase 감지 (LSTM 모델)
- 각도 계산 (수학 로직)
- 진단 생성 (Thresholds 규칙 매칭)
- JSON 응답 반환

### ❌ 담당하지 않는 것
- 사용자 인증 → NestJS가 처리
- DB 저장 → NestJS가 처리
- 골프장 검색 → NestJS + Redis
- 프론트엔드 렌더링 → Mobile App

**핵심: 이 서비스는 완전히 Stateless!**

---

## 데이터 흐름

### 1️⃣ 사용자가 스윙 영상 업로드
```
[Mobile] → [NestJS API] POST /swing-analysis
  Body: { video: File, userId: 123 }
```

### 2️⃣ NestJS가 AI 서비스 호출
```typescript
// NestJS 코드 예시
const formData = new FormData();
formData.append('file', videoFile.buffer, 'swing.mp4');
formData.append('side', 'right');
formData.append('club', 'iron');

const aiResponse = await this.httpService.post(
  `${AI_SERVICE_URL}/analyze`,
  formData,
  { timeout: 30000 }
).toPromise();
```

### 3️⃣ FastAPI(여기)에서 분석
```python
# app/analyze/services.py
def analyze_swing(file_path, side, club):
    # 1. 전처리
    preprocessed = preprocess_video(file_path)
    
    # 2. 골격 추출
    landmarks = PoseExtractor().extract(preprocessed)
    
    # 3. Phase 감지
    phases = detect_phases(landmarks)  # LSTM
    
    # 4. 각도 계산
    metrics = calculate_metrics(landmarks, phases)
    
    # 5. 진단 생성
    diagnosis = apply_thresholds(metrics, club)
    
    return {
        "swingId": uuid4(),
        "metrics": metrics,
        "phases": phases,
        "diagnosis_by_phase": diagnosis
    }
```

### 4️⃣ 결과 반환 및 저장
```
[FastAPI] → JSON → [NestJS]
                       ↓
                  [MySQL 저장]
                       ↓
                  [Mobile에 응답]
```

---

## Phase 감지 알고리즘

### ML 기반 (기본)
```python
# app/ml/phase_lstm.py
class PhaseLSTM(nn.Module):
    """
    입력: (batch, seq_len, features)
    - features: elbow, knee, spine_tilt 등
    
    출력: (batch, 7)  # P2~P9 중 7개 Phase 인덱스
    """
    def __init__(self):
        self.lstm = nn.LSTM(input_size=6, hidden_size=64, num_layers=2)
        self.fc = nn.Linear(64, 7)
```

**학습 데이터:**
- `data/raw/pros/`: 프로 골퍼 스윙 영상
- `scripts/video_to_csv.py`로 변환
- Phase 라벨은 수동 태깅

---

## Thresholds 시스템

### 설정 파일 구조
```json
{
  "default": {
    "elbow_avg": { "bins": [...] }
  },
  "clubs": {
    "driver": {
      "elbow_avg": { "bins": [...] }  // driver 전용 규칙
    }
  }
}
```

### 적용 로직
```python
# 1. default 규칙 로드
rules = thresholds["default"]

# 2. 클럽별 규칙 병합
if club in thresholds["clubs"]:
    rules.update(thresholds["clubs"][club])

# 3. 각 메트릭에 적용
for key, value in metrics.items():
    if key in rules:
        diagnosis[key] = match_bins(value, rules[key]["bins"])
```

---

## 배포 아키텍처 (향후)

```
┌────────────────────────────────────────┐
│           AWS Cloud                    │
├────────────────────────────────────────┤
│                                        │
│  ┌─────────────┐                      │
│  │     ALB     │                      │
│  └─────────────┘                      │
│         │                              │
│         ▼                              │
│  ┌─────────────────────────────┐      │
│  │       ECS Cluster           │      │
│  │  ┌──────────┐ ┌──────────┐  │      │
│  │  │ NestJS   │ │ FastAPI  │  │      │
│  │  │ Task     │ │ Task     │  │      │
│  │  └──────────┘ └──────────┘  │      │
│  └─────────────────────────────┘      │
│         │              │              │
│         ▼              ▼              │
│  ┌──────────┐   ┌──────────┐         │
│  │   RDS    │   │ElastiCache│         │
│  │  MySQL   │   │   Redis   │         │
│  └──────────┘   └──────────┘         │
│                                        │
└────────────────────────────────────────┘
```

**관리:** [platform-infra](https://github.com/wjdghtls95/platform-infra) Terraform

---

## 성능 고려사항

### 현재 상태
- CPU 모드 (MediaPipe)
- 동기 처리
- 1개 요청당 약 5~8초

### 병목 지점
1. **전처리 (FFmpeg)**: 2~3초
2. **골격 추출 (MediaPipe)**: 2~3초
3. **Phase 감지 (LSTM)**: 0.5초
4. **각도 계산**: 0.2초

### 향후 최적화 계획
- [ ] 비동기 큐 (Celery) → 동시 처리 10배
- [ ] GPU 가속 → 분석 시간 60% 단축
- [ ] 프레임 샘플링 → 처리 속도 40% 향상

---

## 다음 단계

- [📖 API Reference](api.md)
- [📖 Development Guide](dev_guide.md)
- [📖 Module Reference](module_reference.md)
